...
export default function MainProfileScreen({ navigation }) {
    ...
    return (
        ...
        <ScrollView contentContainerStyle={styles.MainProfilecontainer}>
...
<View style={{flex:1}}>
                    <View style={styles.profileinfoBoxWrapper}>
                        ...
                        <View style={styles.profileinfoBox}>
                            ...
                            <LinearGradient
                                // colors={["#b2d8d8", "#006666"]}
                                colors={["#008080", "#006666"]}

                                style={{ height: "100%" }}>

                                <View style={{ flex: 1, flexDirection: 'row', paddingVertical: wp('2%'), paddingHorizontal: wp("4%") }}>

                                    <View style={{ flex: 1, justifyContent: 'center' }}>
                                        <Avatar.Image
                                           //chang:::::: source={{ uri: (personalapidata.length > 0 ? (personalapidata[0].EmployeePic ? `data:image/png;base64,${personalapidata[0].EmployeePic}` : (personalapidata[0].Gender.toUpperCase() == "M" || personalapidata[0].Gender.toUpperCase() == 'MALE' ? Contants.USER_IMAGE.MALE : Contants.USER_IMAGE.FEMALE)) : 'N/A') }}
                                            source={{ uri: (Contants.USER_IMAGE.MALE) }}
                                    ...
                                    <View style={{ flex: 2.5, flexDirection: 'column', justifyContent: 'center' }}>

                                        {/*chang:::: <Text style={styles.title}>{personalapidata.length > 0 ? (personalapidata[0].EmpName == null || personalapidata[0].EmpName == '' ? 'N/A' : personalapidata[0].EmpName) : 'N/A'} </Text> */}
                        ...
                        <View style={{ flex: 1, paddingHorizontal: wp('2.5%'), paddingVertical: wp("2%") }}>
                            <View style={{ flexDirection: 'row', marginTop: Platform.isPad ? wp('3%') : wp('5%'), paddingLeft: wp('2.5%'), alignItems: 'center' }}>
                                ...
                                {/*change::: <Text style={styles.profileinfotext}>{personalapidata.length > 0 ? (personalapidata[0].Location == null || personalapidata[0].Location == '' ? 'N/A' : personalapidata[0].Location) : ''}</Text> */}
                            ...
                            <View style={{ flexDirection: 'row', marginTop: Platform.isPad ? wp('3%') : wp('4%'), paddingLeft: wp('2.5%') }}>
                                ...
                                {/*Change::: <Text style={styles.profileinfotext}>{personalapidata.length > 0 ? (personalapidata[0].Cell == null || personalapidata[0].Cell == '' ? 'N/A' : personalapidata[0].Cell) : ''}</Text> */}
                            ...
                            <View style={{ flexDirection: 'row', marginTop: Platform.isPad ? wp('3%') : wp('4%'), paddingLeft: wp('2.5%') }}>
                                ...
                                {/*change::: <Text style={styles.profileinfotext}>{personalapidata.length > 0 ? (personalapidata[0].EmployeeContactEmail == null || personalapidata[0].EmployeeContactEmail == '' ? 'N/A' : personalapidata[0].EmployeeContactEmail) : ''}</Text> */}
...
const styles = StyleSheet.create({
    ...
    title: {
        marginTop: wp('2%'),
        fontSize: Platform.isPad ? wp('3%') : wp('5%'),
        fontWeight: 'bold',
        color: '#fff'
    },
    caption: {
        fontSize: Platform.isPad ? wp('3%') : wp('4%'),
        fontWeight: '500',
        marginTop: wp('1%'),
        marginBottom: wp('1%'),
        color: "#fff",
